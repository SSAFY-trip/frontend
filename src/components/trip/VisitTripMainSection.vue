...
export default {
     ...
     methods: {
          ...
          formatTripDateRange(startDate, endDate) {
               ...
               const dayDifference = Math.round((end - start) / (1000 * 60 * 60 * 24));
               const nights = dayDifference; // Number of nights
               ...
               return `${startFormatted} ~ ${endFormatted} (${nights}박 ${days}일)`;
          },
     }
}
</script>
...
.image-shape-container {
     ...
     mask-image: url('@/assets/shapes/trip-main-img.svg');
